...
Datastore.prototype.max = function(query, property , cbFn) {
  var cursor = new Cursor(this, query, function(err, docs, cbCursor) {
	...
	try{
	  ...
	  //todo: property is object(projection) return { property : max(doc.property) , ... }  
	  
	  if (property.substring(0,1) === '$') {
		rValue =  _.max( docs ,  function(doc){ return doc[ property.substring(1) ] }   );
	  }else{
		rValue =  _.max( _.pluck(docs, property) );
	  }
...
Datastore.prototype.min = function(query, property , cbFn) {
  var cursor = new Cursor(this, query, function(err, docs, cbCursor) {
	if (err) { return cbCursor(err); }
	try{
	  ...
	  //todo: property is object(projection) return { property : min(doc.property) , ... }  
	  
	  if (property.substring(0,1) === '$') {
		rValue =  _.min( docs ,  function(doc){ return doc[ property.substring(1) ] }   );
	  }else{
		rValue =  _.min( _.pluck(docs, property) );
	  }
