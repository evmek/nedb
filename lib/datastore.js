...
/* ---------------------------------------------------------------------------- 
		todo: 
   ---------------------------------------------------------------------------- */
//Datastore.prototype._count =  Datastore.prototype.count;  
   
var aggragateFunctions ={
		//"count": function() {} ,
		"min": function(docs,p) { 
				var t ;
				if (p.substring(0,1) === '$') {
					t  _.min( docs ,  function(doc){ return doc[ p.substring(1) ] }   );
			  }else{
			  	t =  _.min( _.pluck(docs, p) );
			  }
			  return t;
		} 
		,"max": function(docs,p) { 
				var t ;
				if (p.substring(0,1) === '$') {
					t =  _.max( docs ,  function(doc){ return doc[ p.substring(1) ] }   );
			  }else{
			  	t =  _.max( _.pluck(docs, p) );
			  }
			  return t;
		}
		,"sum": function(docs,p) {
				var t ;
				_.pluck(docs, p).forEach(function(v){
						if(! _.isNumber(v)) {
							throw p + " type is not number" 
							return;
						}
						t=+v;
				});
		...
		,"avg" : function(docs,p) {
				var t , sum = aggragateFunctions["sum"](docs,p) ; 
				t = sum / docs.length;
				return t;
		}
		,"stdev":function(docs,p) {}
		,"countdistinct":function(docs,p) {}
		,"rank" : function(docs,p) {}
	}	;
	
Datastore.prototype.aggragate = function( aggragate, query, property , cbFn) {
	
	var aggFn = aggragateFunctions[aggragate];
	
	var cursor = new Cursor(this, query, function(err, docs, cbCursor) {
		if (err) { return cbCursor(err); }
		try{
		  var 
		  	pa = util.isArray(property) 
		  	, rValue = null
		  ;
...
Object.keys(aggragateFunctions).forEach(function (k) {
  Datastore.prototype[k] = function(){
		return this.aggragate(k,arguments); 	
  };
});
